...
async def get_or_create_response(
    ...
    coll = db["responses"]
    sid_variants = id_match_variants([survey_id])
    doc = await coll.find_one(
        {
            "survey_id": {"$in": sid_variants},
    ...
    now = datetime.utcnow()
    try:
        canonical_sid = oid_str(parse_oid(str(survey_id).strip()))
    except Exception:
        canonical_sid = str(survey_id).strip()
    new_doc = {
        "survey_id": canonical_sid,
        "user_id": user_id,
        "session_id": session_id,
        "answers": [],
        "status": "in_progress",
...
async def compute_next(
    ...
    last_qid: Optional[str] = (
        oid_str(answers[-1].get("question_id")) if answers else None
    )
...
async def save_answer(
    ...
    for i, a in enumerate(answers):
        if oid_str(a.get("question_id")) == oid_str(question_id):
...
async def submit_survey(
    ...
    coll = db["responses"]
    sid_for_match = id_match_variants([survey_id])
    flt_completed: Dict[str, Any] = {
        "survey_id": {"$in": sid_for_match},
        "status": "completed",
    }
    if user_id:
