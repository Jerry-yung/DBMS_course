...
| `lineage_id` | 可选，题库家族 ID；从题库选用时写入，用于跨问卷统计与使用方查询 |
| `source_library_version_id` | 可选，选用时的 `library_questions._id` |
| `has_jump_rules` | 是否有启用的跳转规则 |
...
| 字段 | 说明 |
|------|------|
| `_id` | ObjectId，API 字符串 `id` |
| `owner_id` | 所有者用户 ID 字符串 |
| `lineage_id` | 逻辑题家族 ID（同一家族各版本相同） |
| `parent_version_id` | 可选，上一版本文档 ID 字符串 |
| `title` / `type` / `required` / `options` / `validation` | 与问卷内题目结构一致；插入后语义字段不就地更新 |
| `created_at` | 创建时间 |
...
| 字段 | 说明 |
|------|------|
| `grantor_id` | 共享发起用户 ID |
| `grantee_id` | 被共享用户 ID |
| `lineage_id` | 被共享的题库家族 ID |
| `created_at` | 创建时间 |
...
| `preset_kind` | 可选；`shared_inbox` 表示系统预置「共享库」（不可删、不可改名；描述固定为来自他人共享） |
| `created_at` | 创建时间 |
...
|------|------|
| `bank_id` | 所属题库 ID 字符串 |
| `lineage_id` | 入池题目的家族 ID（同一题库内按 lineage 去重） |
| `added_at` | 加入时间 |
...
| library_questions | 题库题目版本（不可变内容） |
| question_shares | 按 `lineage_id` 将题库使用权授予其他用户 |
| question_banks | 题库（容器） |
| question_bank_items | 题库内条目，按 `lineage_id` 指向题目家族，详情返回该家族最新版本 |
...
开发前请执行：`python scripts/init_db.py`（在项目根目录下，已配置 `sys.path` 的脚本自处理）。

题库与版本规则见 `docs/QUESTION_BANK.md`。
